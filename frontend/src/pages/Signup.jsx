...
export const Signup = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            window.localStorage.setItem('username',response.data.newUser.username)
            setUser({
            ...
            setTimeout(() => setError(null),4000)
            console.log('Failed to sign up : ',error.response ? error.response.data.message : error.message);
        }
    }
    ...
    return (
        <div className='h-screen flex'>
            ...
            <div className='absolute mx-1 mt-1 mb-1 md:ml-0 md:mt-0 md:mx-0 md:mb-0 w-[98vw] h-[99vh] sm:w-[99vw]  sm:h-10/12 md:h-full md:static rounded-md md:rounded-none md:w-1/2 bg-black flex flex-col items-center'>
                ...
                <div className='text-blue-400 text-2xl mt-5 md:text-3xl md:mt-20 font-semibold '>Sign up</div>
                <div className='md:text-lg text-blue-200 md:mt-2'>Create your account.</div>
                <form onSubmit={handleSubmit} className='flex flex-col items-center gap-5 w-full px-3 mt-14 md:w-1/2 md:mt-10'>
                    ...
                    <Button className='bg-blue-500 hover:bg-blue-600 active:bg-blue-700 p-3'>Sign up</Button>
                </form>
                <div className='text-blue-200 mt-2'>Already have an account? <Link to='/signin' className='hover:underline text-blue-400'>Sign in</Link>.</div>
